...
public class PlayerController implements Initializable {
    ...
    private void handleReady() {
        ...
        timeSlider.valueChangingProperty().addListener((ObservableValue<? extends Boolean> slider, Boolean oldVal, Boolean newVal)->{
            if(mediaPlayer != null){
                double newct = timeSlider.getValue();
                mediaPlayer.seek(Duration.millis(newct));
            }
        });
    }
    ...
    private String round(Duration time){//all this and it doesn't work, ugh
        double temp1 = time.toMillis();
        otherRound(temp1, 1);
        float temp2 = (float)temp1/1000;
        String rounded = String.valueOf(temp2);
        return rounded;
    }
    
    private static double otherRound (double value, int precision) {
